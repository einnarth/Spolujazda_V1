...
export default function Home() {
  ...
  async function fetchRides() {
    const { data } = await supabase.from('rides').select('*').order('date', { ascending: false });
    if (data) {
      setRides(data.map(r => ({
        id: r.id,
  ...
  if (loading) return <div className="min-h-screen bg-brand-bg flex items-center justify-center text-white italic font-black uppercase tracking-widest">Načítavam...</div>;
      ...
      <main className="min-h-screen bg-brand-bg text-gray-400 p-4 md:p-8 font-sans italic">
        <div className="max-w-7xl mx-auto space-y-10">
          ...
          <header className="flex justify-between items-end border-b border-brand-border pb-8">
            <div>
              <h1 className="text-4xl font-black italic tracking-tighter text-white uppercase leading-none">Ride Tracker</h1>
              <div className="flex items-center gap-2 mt-3">
                <div className={`w-2 h-2 rounded-full ${isAdmin ? 'bg-blue-500 shadow-[0_0_10px_#3b82f6]' : 'bg-green-500'}`}></div>
                <span className="text-[10px] uppercase tracking-[0.3em] font-bold text-gray-500">{isAdmin ? 'Admin Mode' : `Pasažier: ${currentUserPassenger?.name}`}</span>
              </div>
            </div>
            <button onClick={() => supabase.auth.signOut()} className="text-[10px] text-gray-600 hover:text-white uppercase tracking-widest font-black transition-colors">[ Odhlásiť ]</button>
          </header>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                <div className="lg:col-span-4 space-y-8">
                  ...
                  <div className="bg-brand-card p-8 rounded-5xl border border-brand-border shadow-2xl">
                    ...
                    <div className="flex justify-center"><CalendarView selectedDate={selectedDate} onDateChange={setSelectedDate} /></div>
                  </div>

                  <button onClick={async () => {
                    if (!selectedPassenger || !selectedDate) return alert("Vyber pasažiera!");
                    const { error } = await supabase.from('rides').insert([{ date: selectedDate.toISOString().split('T')[0], passengerName: selectedPassenger.name, price: parseFloat(price) }]);
                    if (error) alert(error.message); else fetchRides();
                ...
                <div className="lg:col-span-8">
                  <div className="bg-brand-card p-10 rounded-5xl border border-brand-border shadow-2xl min-h-150">
                    <h2 className="text-2xl font-black italic uppercase tracking-tighter mb-10 text-white border-l-4 border-blue-600 pl-6">Prehľad dlhov</h2>
                    <DebtList rides={rides} />
                  </div>
                </div>
              </div>
          ) : (
              <div className="max-w-3xl mx-auto bg-brand-card p-10 md:p-16 rounded-5xl border border-brand-border shadow-2xl">
                <h2 className="text-3xl font-black italic uppercase tracking-tighter mb-10 text-white">Tvoje jazdy</h2>
                <DebtList rides={rides.filter(r => r.passengerNames.includes(currentUserPassenger?.name || ''))} />
              </div>
          )}
        </div>
      </main>
